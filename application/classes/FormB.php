...
class FormB extends Base {
	...
	public function __construct($id = "pfbc", $width = "") {
		$this->configure(array(
			...
			$this->resourcesPath = js_common."plugins/forms/Resources";
	}
	...
	private function renderJS() {
		...
		window.CKEDITOR_BASEPATH="'.$this->getResourcesPath().'/ckeditor/";
		</script>';
		$this->renderJSFiles();
		...
		else 	echo 'jQuery(this).find("input[type=submit]").attr("disabled", "disabled");';
		echo '});';

		/*jQuery is used to set the focus of the form's initial element.*/
		if(!in_array("focus", $this->prevent))	echo 'jQuery("#', $id, ' :input:visible:enabled:first").focus();';

		$this->view->jQueryDocumentReady();
		foreach($this->elements as $element) $element->jQueryDocumentReady();
		...
		if(!empty($this->ajax)) {
			...
			jQuery.ajax({
						url:app.editFormAction, //"{$this->attributes["action"]}",
				type: "{$this->attributes["method"]}",
				data: jQuery("#$id").serialize(),
				success: function(response) {app.vent.trigger("load:end");
